...
def crear_mapa_ids(conn, tabla, id_col):
...
try:
    ...
    print("--- Cargando Ficha 1: factVehicleAccident ---")
    ...
    if not nuevos_archivos_csv:
        print("No se encontraron archivos CSV nuevos para Ficha 1. El proceso ha finalizado.")
        conn.close()
        exit()
    ...
    # FKs de Hechos
    map_ids_accidents = crear_mapa_ids(conn, "factAccident", "idAccident")
    
    # FKs de Dimensiones (Puentes)
    map_ids_service = crear_mapa_ids(conn, "dim_ServiceType", "idServiceType")
    map_ids_vehicletypevalue = crear_mapa_ids(conn, "dim_VehicleTypeValue", "idVehicleTypeValue")
    map_ids_maneuver = crear_mapa_ids(conn, "dim_ManeuverType", "idManeuverType")
    map_ids_consequence = crear_mapa_ids(conn, "dim_ConsequenceType", "idConsequenceType")
    ...
    print(f"Mapas creados. {len(map_ids_accidents)} accidentes válidos encontrados.")
    ...
    for archivo_csv in nuevos_archivos_csv:
        ...
        try:
            ...
            print(f" ERROR: No se pudo leer {archivo_csv}. Error: {e}")
        ...
        try:
            ...
            if file_had_errors:
                dirty_files.add(os.path.basename(archivo_csv))
                conn.rollback()
                print(f" ADVERTENCIA: Se encontraron errores en {os.path.basename(archivo_csv)}. Se revirtió la carga de este archivo.")
            else:
                conn.commit()
                print(f" Archivo {os.path.basename(archivo_csv)} cargado exitosamente.")

        except Exception as e_file:
            # Error a nivel de TRANSACCIÓN/ARCHIVO
            conn.rollback()
            print(f"  ERROR de Base de Datos al procesar {os.path.basename(archivo_csv)}: {e_file}.")
            dirty_files.add(os.path.basename(archivo_csv))
    ...
    print("...procesamiento de Ficha 1 finalizado.")

    print("Registrando archivos procesados en el log (Ficha 1)...")
    ...
    if files_to_log:
        ...
        print(f"{len(files_to_log)} archivos 100% limpios registrados en el log etl_log_ficha1_vehiculos.")
    
    if dirty_files:
        print(f"ADVERTENCIA: {len(dirty_files)} archivos con errores no se registraron en etl_log_ficha1_vehiculos y serán reintentados.")

    if failed_rows_details:
        print("\n--- ⚠️ Reporte Detallado de Errores de Carga (Ficha 1) ---")
        print(f"Se omitieron {len(failed_rows_details)} filas por errores de FK, validación o de BD.")
        for i, (fila, error, archivo) in enumerate(failed_rows_details[:10]): # Mostrar solo los primeros 10
            print(f"\n - Archivo: {archivo}")
            print(f" Error: {error}")
            if fila:
                print(f" Datos: ID Accidente {fila.get('ID Accidente')}, Patente {fila.get('Patente')}")
                
    conn.commit() # Commit final para los logs
    print("Proceso ETL para Ficha 1 completado.")
